...
public class Btree {
    ...
    //print lowest common ancestor of the nodes
    ...
    //print the path to any node from the root
    ...
    //print diameter of the tree(O(n2))
    ...
    //print diameter of the tree O(n) 
    ...
    //print all nodes K distance away from the given node in downward direction
    ...
    //O(n) complexity withh more space 
    ...
    public static class allpair{
        ...
        int ceil = Integer.MAX_VALUE;//(int)1e8
        int floor = (int)-1e8;
        Node pred = null;
        Node succ = null;
        Node prev = null;
    ...
    public static void allSol(Node node, int level, int data, allpair sol){
        ...
        if(node.data == data && sol.pred == null && sol.prev != null){
            sol.pred = sol.prev;
        }

        if(sol.prev.data == data && sol.succ == null && sol.prev != null){
            sol.succ = node;
        }
    ...
    //check if a tree is bst or not using inorder traversal
...
public static Node createBST(int[] arr,int si,int ei){
 ...
 public static Node lcaBST(Node node, int a, int b){
 ...
 public static Node range(Node node, int a, int b){
    if(node == null){
        return null;
    }
 ...
 public static int width(Node node, boolean isleft){
     if(node == null) return -1;

     int left = width(node.left, isleft) + (isleft? 1: -1);
     int right = width(node.right, isleft) + (isleft? -1: 1);
...
// //  }
 ...
 static int maxlen =0;
 public static void LCseq(Node node, int potentialValue, int currlen){
     ...
     LCseq(node.left, node.data +1, currlen);
     LCseq(node.right, node.data +1, currlen);
 }

//  public static void cameraTree(Node node, String ans){
...
//Traversal of trees without recursion using stacks
//PRE-ORDER
